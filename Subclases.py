...
# Variables globales para el modo historia
vida = 100
fortuna = 0
ejercito = 0
murallas = 0
...
def modo_historia():
    ...
    class Juego:
        ...
        def print_pause(self, mensaje):
            print(mensaje)
            time.sleep(1.5)

        def mostrar_estado(self):
            self.print_pause(f"\n--- Estado actual ---")
            self.print_pause(f"Vida: {self.vida}")
            self.print_pause(f"Fortuna: {self.fortuna}")
            self.print_pause(f"Ejército: {self.ejercito}")
            self.print_pause(f"Murallas: {self.murallas}")
            self.print_pause("---------------------")

        def batalla(self, jugador_vida=300, enemigo_vida=80):
            ...
            while jugador.esta_vivo() and enemigo.esta_vivo():
                ...
                print("2. Ataque Especial (50 de daño)" if not especial_usado else "2. Escapar")
        ...
        def elegir_bayas(self):
            ...
            if decision == '1':
                ...
                planta = input("¿Cuál eliges? (1/2/3): ")
                self.print_pause("1. Las rojas")
                self.print_pause("2. Las azules")
                self.print_pause("3. O las negras")
                if planta == '2':
                    self.vida += 10
        ...
        def comerciantes_mayas(self):
            ...
            self.mostrar_estado()
            if vida <= 0:
                self.print_pause("Has muerto en el cenote.")
                return False
            return True

        def ruta_comercial(self):
            self.print_pause("De vuelta en Perú, decides comerciar en la ruta comercial maya.")
        ...
        def agricultura(self):
            self.print_pause("Te estableces en el Valle Sagrado y comienzas a trabajar en la agricultura.")
        ...
        def cortejar_inca(self):
            ...
            decision = input("¿Qué haces? (1/2/3): ")
            if decision == '1':
        ...
        def proteger_dominios(self):
            ...
            if decision == '1':
                ...
                self.print_pause("Atacas a los españoles, pero su superioridad táctica te derrota.")
                self.print_pause("Tu poder se desvanece.")
            self.mostrar_estado()

        def jugar(self):
            self.intro()
            if (self.elegir_bayas() and self.batalla() and
                self.comerciantes_mayas() and self.batalla() and
                self.ruta_comercial() and self.batalla() and
                self.agricultura() and self.batalla() and
                self.cortejar_inca() and self.batalla()):
                self.proteger_dominios()

        def reiniciar_juego(self):
            self.__init__()  # Reinicia los atributos
    ...
    if __name__ == "__main__":
        while True:
            juego = Juego()
            juego.jugar()
            if input("\n¿Quieres jugar de nuevo? (sí/no): ").lower() != 'sí':
                break
...
def modo_batalla():
    ...
    class Personaje:
        def __init__(self, nombre, significado, tipo, vida, ataque, velocidad, alcance, especial, armas):
            ...
            self.armas = armas if armas else ["Puños"]  # Arma por defecto si no hay armas
            self.arma_actual = self.armas[0]
        ...
        def usar_especial(self, objetivo=None):
            ...
            if objetivo is None and self.nombre != "Koka":
                print("No hay objetivo para la habilidad especial.")
                return False
            ...
            if self.nombre == "Aymara":
                danio = 300
                objetivo.vida = max(0, objetivo.vida - danio)
